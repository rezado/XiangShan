  ...
  with HasPerfEvents
{
  ...
  val s0pf = WireInit(VecInit(Seq.fill(Width)(false.B)))
  val s0gpf = WireInit(VecInit(Seq.fill(Width)(false.B)))
  val s0af = WireInit(VecInit(Seq.fill(Width)(false.B)))
  ...
  (0 until Width).foreach{i =>
    ...
    for (d <- 0 until nRespDups) {
      ...
      perm_check(perm(i)(d), req_out(i).cmd, i, d, g_perm(i)(d), req_out(i).hlvx, req_out_s2xlate(i), s0pf(i), s0gpf(i), s0af(i))
    }
  ...
  def perm_check(perm: TlbPermBundle, cmd: UInt, idx: Int, nDups: Int, g_perm: TlbPermBundle, hlvx: Bool, s2xlate: UInt, s0pf: Bool = false.B, s0gpf: Bool = false.B, s0af: Bool = false.B) = {
    ...
    when (RegNext(s0pf || s0gpf || s0af)) {
      resp(idx).bits.excp(nDups).pf.ld := RegNext(s0pf) && isLd
      resp(idx).bits.excp(nDups).pf.st := RegNext(s0pf) && isSt
      resp(idx).bits.excp(nDups).pf.instr := false.B

      resp(idx).bits.excp(nDups).gpf.ld := RegNext(s0gpf) && isLd
      resp(idx).bits.excp(nDups).gpf.st := RegNext(s0gpf) && isSt
      resp(idx).bits.excp(nDups).gpf.instr := false.B

      resp(idx).bits.excp(nDups).af.ld := RegNext(s0af) && TlbCmd.isRead(cmd)
      resp(idx).bits.excp(nDups).af.st := RegNext(s0af) && TlbCmd.isWrite(cmd)
